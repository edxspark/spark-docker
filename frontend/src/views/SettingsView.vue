...
const FIELD_META: Record<string, FieldMeta> = {
  ...
  'video.burn_subtitles': { label: '烧录中文字幕', type: 'switch', help: '把中文字幕直接渲染进画面，抖音等平台更吃字幕' },
  'video.subtitle_font_size': { label: '字幕字号', type: 'number', min: 8, max: 72 },
  'video.subtitle_font_name': { label: '字幕字体', type: 'text', help: '留空自动选择系统中可用的中文字体。注意 macOS 的 PingFang SC 无法被 libass 加载，填写后可能触发字体回退' },
  'video.subtitle_margin_v': { label: '字幕底部边距', type: 'number', min: 0, max: 400 },
  'video.keep_bgm': { label: '保留原视频背景音', type: 'switch', help: '保留原音轨并压低音量作为氛围垫底，纯人声感更强时建议保持开启' },
  'video.bgm_volume': { label: '背景音量倍率', type: 'number', min: 0, max: 2, step: 0.02 },
